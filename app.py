...
def explode_commodities(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 3. Handle cases where the commodity list is not split by amount (e.g., 'Wheat, Paddy, & Cotton')
    # If the format IS NOT 'Amount Commodity, Amount Commodity', we distribute the total amount equally.

    # 4. If the commodity list contains ONLY NAMES (no numeric amounts), split total amount equally
    # This logic is simplified from the old report_utils, assuming simple lists like "Wheat, Cotton"
    
    # Check if any item contains digits at the start (to differentiate simple lists from old format)
    has_split_amounts = df_exploded["commodity_items"].str.match(r"^\s*\d+").any()

    if has_split_amounts:
        # Complex logic from old report_utils (for "200 Edible oil, 300 Paddy")
        pattern = r"^\s*(\d+)\s*(.*)"
